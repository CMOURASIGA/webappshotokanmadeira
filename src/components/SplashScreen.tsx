...
export function SplashScreen({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer2 = setTimeout(() => setStage(2), 5800);
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#111111] flex flex-col items-center justify-center transition-opacity duration-1000 ${stage === 2 ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
      <div className={`flex flex-col items-center transition-all duration-[1500ms] transform ${stage >= 1 ? "translate-y-0 opacity-100 scale-100" : "translate-y-8 opacity-0 scale-95"}`}>
